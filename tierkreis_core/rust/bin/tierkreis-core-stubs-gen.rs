...
const NODEDEF_CLASS_PATCH: &'static str = "
class NodeDef:
...
// `pyo3_introspection`. So we patch them in manually from a handwritten stub.
const NODESTEP_CLASS_PATCH: &'static str = "
class NodeStep:
...
pub fn main() {
    ...
    for (stub_name, stub) in &stubs {
        ...
        let eq_regex = Regex::new(r"__ne__\(self, /, other: \w+\.\w+").unwrap();
        let patched_stub = eq_regex.replace_all(&patched_stub, "__ne__(self, /, other: object");
