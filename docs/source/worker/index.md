...
The default Rust runtime runs workers through its subprocess executor. Workers
can be defined in the current project or installed as prepackaged dependencies.
...
The project generated by the CLI installs each local implementation package and
its worker command into the project environment. Confirm that command is visible:

```bash
uv run which tkr-worker-name
```

The default runtime finds and invokes this command from `PATH`:
...
The implementation package declares a `tkr-worker-name` command. Running
`which tkr-worker-name` confirms that the default runtime can discover it.
During a workflow run the subprocess executor invokes:
